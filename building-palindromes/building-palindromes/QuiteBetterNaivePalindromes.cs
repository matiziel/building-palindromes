...
namespace building_palindromes
{
    public class QuiteBetterNaivePalindromes : BuildingPalindromes
    {
            ...
            for (int i = - second.Length + 1; i < first.Length; i++)
            {
                ...
                for (int k = 0; k <= reversedSecond.Length && (k + i) <= first.Length; k++)
                {
                    ...
                    if ((k < reversedSecond.Length && (k + i) < first.Length) && first[k + i] == reversedSecond[k])
                        ++longestMatch;
                    else if (longestMatch > 0 && longestMatch != second.Length)
                    {
        ...
        private void AddPalindromesToList(int beginIndex, int endIndex, int longestMatch, string pattern)
        {
            ...
            allPalindromes.Add(BuildString(ReverseString(pattern), firstPalindromes.PalindromesBefore[beginIndex + endIndex - longestMatch], pattern));

            allPalindromes.Add(BuildString(ReverseString(pattern), secondPalindromes.PalindromesAfter[second.Length - endIndex + longestMatch], pattern));
            allPalindromes.Add(BuildString(pattern, secondPalindromes.PalindromesBefore[second.Length - endIndex], ReverseString(pattern)));
        }
